...
def create_plot(df, indicators):
    ...
    for indicator in indicators:
        if indicator == "Bollinger Bands":
            ...
            fig.add_trace(go.Scatter(x = df.index, y=df[df['SQZ_OFF'] == 1], mode = 'markers', marker = dict(color='orange', size=5), name = 'Low Volatility'), row = 4, col =1)
            fig.add_trace(go.Scatter(x = df.index, y=df[df['SQZ_ON'] == 1], mode = 'markers', marker = dict(color='purple', size=5), name = 'High Volatility'),row = 4, col =1)
        elif indicator == "ADX":
