...
def create_plot(df, indicators):
    ...
    for indicator in indicators:
        if indicator == "Bollinger Bands":
            ...
            fig.add_trace(go.Scatter(x = df.index, y=df['SQZPRO_ON_NARROW'], mode = 'markers', marker = dict(color='blue', size=7), name = 'Low Volatility'), row = 4, col =1)
            fig.add_trace(go.Scatter(x = df.index, y=df['SQZPRO_ON_WIDE'], mode = 'markers', marker = dict(color='purple', size=7), name = 'High Volatility'), row = 4, col =1)
        elif indicator == "ADX":
