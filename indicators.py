...
#             colors = ['blue' if val > 0 else 'orange' for val in df['MACD']]
            fig.add_trace(go.Bar(x=imp_macd.index, y=imp_macd['MACD'], marker_color=['blue' if x > imp_macd['Signal'] else 'orange' for x in imp_macd['MACD']], showlegend = False, name='MACD impulse'),row = 2, col=1)
