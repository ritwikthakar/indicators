...
def create_plot(df, indicators):
    ...
    for indicator in indicators:
        if indicator == "Bollinger Bands":
            ...
            bar_colors = ['blue' if x < imp_macd['Signal'] else 'orange' for x in imp_macd['MACD']]
            fig.add_trace(go.Bar(x=imp_macd.index, y=imp_macd['MACD'], marker_color=bar_colors, showlegend = False, name='MACD impulse'),row = 2, col=1)
