...
def create_plot(df, indicators):
    ...
    for indicator in indicators:
        if indicator == "Bollinger Bands":
            ...
            fig.add_trace(go.Scatter(x = df[df['SQZ_OFF'] != 0].index, y=df[df['SQZ_OFF'] != 0], mode = 'markers', marker = dict(color='orange', size=5), name = 'Low Volatility'), row = 4, col =1)
            fig.add_trace(go.Scatter(x = df[df['SQZ_ON'] != 0].index, y=df[df['SQZ_ON'] != 0], mode = 'markers', marker = dict(color='purple', size=5), name = 'High Volatility'),row = 4, col =1)
        elif indicator == "ADX":
