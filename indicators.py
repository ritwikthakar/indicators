...
def create_plot(df, indicators):
    ...
    for indicator in indicators:
        if indicator == "Bollinger Bands":
            ...
            line_colors = ['green' if imp_macd.loc[date, 'MACD'] > imp_macd.loc[date, 'Signal'] else 'red' for date in imp_macd.index]
            fig.add_trace(go.Scatter(x=imp_macd.index, y=imp_macd['MACD'], mode='lines', name='Impulsive MACD', line_color=line_colors))
