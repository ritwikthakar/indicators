...
def create_plot(df, indicators):
    ...
    for indicator in indicators:
        if indicator == 'Candlestick Chart':
            ...
            fig.add_trace(go.Scatter(x = df[df['SQZPRO_ON_WIDE'] != 0].index, y=df[df['SQZPRO_ON_WIDE'] != 0]['SQZPRO_20_2.0_20_2_1.5_1'], mode = 'markers', marker = dict(color='yellow', size=5), name = 'Wide Squeeze'), row = 4, col =1)
