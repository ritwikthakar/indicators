...
def create_plot(df, indicators):
    ...
    for indicator in indicators:
        if indicator == 'Candlestick Chart':
            ...
            fig.add_trace(go.Scatter(x = df.index, y=df['EMA_5'], name = '5 EMA', line = dict(color='lightsalmon', width=2), visible='legendonly'), row =1, col = 1)
            ...
            fig.add_trace(go.Scatter(x = df.index, y=df['HMA_55'], line_color ='darkgoldenrod', name = '55 HMA'), row =1, col = 1)
            ...
            fig.add_trace(go.Scatter(x = df.index, y=df['EMA_26'], name = '26 EMA', line = dict(color='blue', width=2)), row =1, col = 1)
            fig.add_trace(go.Scatter(x = df.index, y=df['EMA_50'], name = '50 EMA', line = dict(color='green', width=2), visible='legendonly'), row =1, col = 1)
            fig.add_trace(go.Scatter(x = df.index, y=df['EMA_100'], name = '100 EMA', line = dict(color='orange', width=2), visible='legendonly'), row =1, col = 1)
...
indicators = ['Candlestick Chart', 'Heikin Ashi Candles', 'RSI' ,'RSI Divergence', 'MACD', 'ATR', 'ADX', 'PSAR', 'Supertrend', 'Fast Double Supertrend', 'Slow Double Supertrend', 'SMA Ribbons', 'Bollinger Bands', "Zero Lag MA Ribbons", "Keltner Channels", "Squeeze Momentum Indicator Pro", "Swing High Swing Low","QQE MOD", "Stochastic RSI", "Stochastic Oscillator", "M Stochastic" ,"Hull Moving Averages", "EMA Ribbons", "EMA Crossover", "Mulit Moving Averages Strategy" ,"MACD Moving Averages" ,"200 EMA", "200 SMA", "100 HMA", "200 HMA", "240 ZLMA", 'Market Bias', "Awesome Oscillator", "Donchian Channels", 'Z Score',"Gann High Low", "Alligator" ,"Fractals", "9 Period Fractals" ,"Fibonacci Retracements", "Fibonacci Extensions", "TD Sequential", "Linear Regression", "Know Sure Thing", "Relative Vigor Index" ,"Half Trend", "Decycler" ,"Engulfing Candles", "Doji Candles", "Dragonfly Doji Candles", "Gravestone Doji Candles", "Hammer Candles", "Inverted Hammer Candles", "Morning Star Candles", "Evening Star Candles", "Abandoned Baby Candles", "Hanging Man Candles", "3 White Soldiers", "3 Black Crows", "3 Line Strike", "Shooting Star", "Tristar"]
